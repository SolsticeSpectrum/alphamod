...
public class PlayerHelper {
   ...
   public void writeWaypointsToNBT(File f) {
      ...
      File wp = new File(f, "waypoints.dat");
      try {
         CompressedStreamTools.writeGzippedCompoundToOutputStream(nbttagcompound1, Files.newOutputStream(wpnew.toPath()));
         if (wpold.exists()) {
            wpold.delete();
         }
         wp.renameTo(wpold);
   ...
   public void processCommands(String s) throws Exception {
      ...
      if (split[0].equalsIgnoreCase("item") || split[0].equalsIgnoreCase("i") || split[0].equalsIgnoreCase("give")) {
         ...
         mc.theWorld.worldInfo.setSpawn(x, y, z);
         ...
         ep.attackEntityFrom(this.ep, Integer.MAX_VALUE);
         ...
         do {
            ...
            mc.theWorld.entityJoinedWorld(es);
            if (et != null) {
               es.mountEntity(et);
            }
            et = es;
         ...
         for (int i = 0; i < npcs.size(); i++) {
            ...
            if (type == 0 && e instanceof EntityLiving) {
               ((EntityLiving) e).damageEntity(Integer.MAX_VALUE);
         ...
         int y = MathHelper.floor_double(ep.posY);
         int z = MathHelper.floor_double(ep.posZ);
         WorldGenerator sml = new WorldGenTrees();
         WorldGenerator big = new WorldGenBigTree();
         ...
         if (split[1].equalsIgnoreCase("load")) {
            ...
            this.mc.changeWorld2(new World(new SaveHandler(parent, child.getName(), false), child.getName(), (new Random()).nextLong()), "Changing World to " + message);
            ...
            World world = null;
            if (sd) {
               world = new World(new SaveHandler(parent, child.getName(), false), child.getName(), seed);
            } else {
               world = new World(new SaveHandler(parent, child.getName(), false), child.getName(), (new Random()).nextLong());
            }
            ...
            if (split.length == 2) {
               sendMessage("The current world seed is " + this.mc.theWorld.worldInfo.getRandomSeed());
               return;
            }
            long seed = -1;
            try {
         ...
         for (int i = 0; i < Item.itemsList.length; i++) {
            if (Item.itemsList[i] instanceof Item) {
               if (norepair) {
                  ...
                  if (Item.itemsList[i].isDamagable()) {
         ...
         mc.theWorld.entityJoinedWorld(tnt);
         ...
         sendMessage("Current Biome: " + mobspawnerbase.biomeName);
         ...
         (new Teleporter()).func_4108_c(mc.theWorld, mc.thePlayer);
         ...
         for (int j = 0; j < i; j++) {
            ...
            mc.theWorld.entityJoinedWorld(entity1);
         }
         ...
         Entity entity;
         for (Iterator iterator = list.iterator(); iterator.hasNext(); entity.setEntityDead()) {
   ...
   public void confuse(EntityCreature entitycreature, EntityCreature entitycreature1) {
      entitycreature.setTarget(entitycreature1);
   }
    ...
    * 
    * @param sourceFile The origin file
