...
public class CodecJOrbis implements ICodec {
	...
	private byte[] decodeCurrentPacket() {
		...
		while(var5 < this.convertedBufferSize) {
			int var2 = this.jorbisDspState.synthesis_pcmout(this.pcmInfo, this.pcmIndex);
			if(var2 <= 0) {
				break;
			}
			...
			for(int var6 = 0; var6 < this.jorbisInfo.channels; ++var6) {
				...
				for(int var8 = 0; var8 < var3; ++var8) {
					...
					if(var9 > Short.MAX_VALUE) {
						var9 = Short.MAX_VALUE;
					}

					if(var9 < Short.MIN_VALUE) {
						var9 = Short.MIN_VALUE;
					}
